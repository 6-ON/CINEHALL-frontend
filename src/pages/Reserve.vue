			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					if (storageToken ?? false) {
						...
						axios.get('http://localhost:8080/users/get-info').then((res1) => {
							data.append('idUser', res1.data.id)
							...
							axios({
								...
								Swal.fire({
									icon: reserveResponse.data.type,
									title: reserveResponse.data.content.message ?? 'error'
								})
							}).catch(err => {
								console.log(err)
							})
